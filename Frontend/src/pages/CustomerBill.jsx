...
import PaymentHeader from "../components/customer/customerPayment/PaymentHeader";
import SellsSummaryCard from "../components/customer/customerPayment/SellsSummaryCard";
import SellsCard from "../components/customer/customerPayment/SellsCard";
import PaymentModal from "../components/customer/customerPayment/PaymentModal";
...
const CustomerBill = () => {
  ...
  const fetchPaymentData = async () => {
    try {
      ...
      const res = await axios.post(
        `${BASE_URL}/admin/v1/get-customer-milk-sells/${customerId}`,
        {
          startDate,
          endDate,
        },
  ...
  return (
    <div className="min-h-screen bg-[#f8f9ff] pb-24">
      <PaymentHeader
        title={customer?.customerName || "Customer Payment"}
      />
        ...
        <section>
          <div className="flex justify-between items-center mb-4">
            ...
            <span className="text-sm text-blue-600 font-semibold">
              {paymentData?.sells?.length || 0} Records
            </span>
          </div>

          {paymentData?.sells?.length > 0 ? (
            <div className="space-y-4">
              {paymentData.sells.map((sell) => (
                <SellsCard
                  key={sell.sellId}
                  sells={sell}
                />
            ...
            <div className="bg-white rounded-2xl border border-gray-200 py-16 text-center">
              ...
              <p className="text-gray-500 mt-2">
                No milk sells are available for the selected
                date range.
              </p>
            </div>
          )}
