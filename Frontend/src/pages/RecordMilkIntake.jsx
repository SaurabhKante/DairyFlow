...
const RecordMilkIntake = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await axios.post(`${BASE_URL}/farmer/v1/milk-purchase/${id}`, payload, {
        headers: {
  ...
  return (
    <div className="min-h-screen bg-[#f8f9ff]">
      ...
      <header className="sticky top-0 bg-white border-b flex items-center gap-4 px-4 h-14">
        ...
        <h1 className="text-xl font-bold text-blue-700">Record Intake</h1>
      </header>
      ...
      <main className="max-w-xl mx-auto p-4 pb-36">
        ...
        <div className="bg-white rounded-xl border p-5 flex gap-4 items-center shadow-sm">
          ...
          <div>
            <p className="text-xs text-gray-500 uppercase">Collecting From</p>

            <h2 className="font-bold text-xl">{farmer?.farmerName}</h2>

            <p className="text-gray-500">{farmer?.mobileNo}</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border mt-6 p-5 ">
          <h3 className="font-semibold mb-4">Collection Quantity</h3>
...
export default RecordMilkIntake;
